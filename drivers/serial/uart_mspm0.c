...
static int uart_mspm0_configure(const struct device *dev, const struct uart_config *cfg)
{
	...
	case UART_CFG_PARITY_NONE:
		data->uart_config.parity = DL_UART_MAIN_PARITY_NONE;
		break;
	case UART_CFG_PARITY_EVEN:
		data->uart_config.parity = DL_UART_MAIN_PARITY_EVEN;
		break;
	case UART_CFG_PARITY_ODD:
		data->uart_config.parity = DL_UART_MAIN_PARITY_ODD;
		break;
	case UART_CFG_PARITY_MARK:
		data->uart_config.parity = DL_UART_MAIN_PARITY_STICK_ONE;
		break;
	case UART_CFG_PARITY_SPACE:
		data->uart_config.parity = DL_UART_MAIN_PARITY_STICK_ZERO;
		break;
	}

	switch (cfg->data_bits) {
	case UART_CFG_DATA_BITS_5:
		data->uart_config.wordLength = DL_UART_MAIN_WORD_LENGTH_5_BITS;
		break;
	case UART_CFG_DATA_BITS_6:
		data->uart_config.wordLength = DL_UART_MAIN_WORD_LENGTH_6_BITS;
		break;
	case UART_CFG_DATA_BITS_7:
		data->uart_config.wordLength = DL_UART_MAIN_WORD_LENGTH_7_BITS;
		break;
	case UART_CFG_DATA_BITS_8:
		data->uart_config.wordLength = DL_UART_MAIN_WORD_LENGTH_8_BITS;
		break;
	}

	switch (cfg->stop_bits) {
	case UART_CFG_STOP_BITS_1:
		data->uart_config.stopBits = DL_UART_MAIN_STOP_BITS_ONE;
		break;
	case UART_CFG_STOP_BITS_2:
		data->uart_config.stopBits = DL_UART_MAIN_STOP_BITS_TWO;
		break;
	}
	data->uart_config.mode = DL_UART_MAIN_MODE_NORMAL;

	switch (cfg->flow_ctrl) {
	case UART_CFG_FLOW_CTRL_RS485:
		data->uart_config.mode = DL_UART_MAIN_MODE_RS485;

	case UART_CFG_FLOW_CTRL_NONE:
		data->uart_config.flowControl = DL_UART_MAIN_FLOW_CONTROL_NONE;
		break;
	case UART_CFG_FLOW_CTRL_RTS_CTS:
		data->uart_config.flowControl = DL_UART_MAIN_FLOW_CONTROL_RTS_CTS;
		break;
	}
	 ...
	 * from the device tree data current-speed
	 */
	retval = clock_control_get_rate(clk_dev, (struct mspm0_sys_clock *)config->clock_subsys,
					&clock_rate);
	if (retval < 0) {
		return retval;
	}

	DL_UART_Main_configBaudRate(config->regs, clock_rate, cfg->baudrate);
