...
LOG_MODULE_REGISTER(bcbus_s, CONFIG_BCBUS_LOG_LEVEL);
...
{
	/* Initialize all BCBUS event counters. */
	ctx->mbs_msg_ctr = 0;
...
static void update_msg_ctr(struct bcbus_context *ctx)
{
	ctx->mbs_msg_ctr++;
}

static void update_crcerr_ctr(struct bcbus_context *ctx)
{
	ctx->mbs_crc_err_ctr++;
}

static void update_excep_ctr(struct bcbus_context *ctx)
{
	ctx->mbs_except_ctr++;
}

static void update_server_msg_ctr(struct bcbus_context *ctx)
{
	ctx->mbs_server_msg_ctr++;
}

static void update_noresp_ctr(struct bcbus_context *ctx)
{
	ctx->mbs_noresp_ctr++;
}
#else
#define bcbus_reset_stats(...)
#define update_msg_ctr(...)
#define update_crcerr_ctr(...)
#define update_excep_ctr(...)
#define update_server_msg_ctr(...)
#define update_noresp_ctr(...)
#endif /* CONFIG_BCBUS_FC08_DIAGNOSTIC */
 ...
 */
static void mbs_exception_rsp(struct bcbus_context *ctx, uint8_t excep_code)
{
 ...
 */
static bool mbs_fc01_coil_read(struct bcbus_context *ctx)
{
	...
	if (ctx->mbs_user_cb->coil_rd == NULL) {
		mbs_exception_rsp(ctx, BCBUS_EXC_ILLEGAL_FC);
		return true;
	}
	...
	if (coil_qty == 0 || coil_qty > coils_limit) {
		...
		mbs_exception_rsp(ctx, BCBUS_EXC_ILLEGAL_DATA_VAL);
		return true;
	}
	...
	while (coil_cntr < coil_qty) {
		...
		if (err != 0) {
			...
			mbs_exception_rsp(ctx, BCBUS_EXC_ILLEGAL_DATA_ADDR);
			return true;
		}
 ...
 */
static bool mbs_fc02_di_read(struct bcbus_context *ctx)
{
 ...
 */
static bool mbs_fc03_hreg_read(struct bcbus_context *ctx)
{
	...
	if ((reg_addr < BCBUS_FP_EXTENSIONS_ADDR) ||
	    !IS_ENABLED(CONFIG_BCBUS_FP_EXTENSIONS)) {
	...
	while (reg_qty > 0) {
		if (reg_addr < BCBUS_FP_EXTENSIONS_ADDR) {
			...
			reg_addr++;
			reg_qty--;
		} else if (IS_ENABLED(CONFIG_BCBUS_FP_EXTENSIONS)) {
			float fp;
			uint32_t reg;
 ...
 *  Register Value        N * 2 Byte
 */
static bool mbs_fc04_inreg_read(struct bcbus_context *ctx)
{
	const uint16_t regs_limit = 125;
	const uint8_t request_len = 4;
	uint8_t *presp;
 ...
 *  Output Value          2 Bytes
 */
static bool mbs_fc05_coil_write(struct bcbus_context *ctx)
{
	const uint8_t request_len = 4;
	...
	if (coil_val == BCBUS_COIL_OFF_CODE) {
		coil_state = false;
	} else {
		coil_state = true;
	}
 ...
 *  Register Value        2 Bytes
 */
static bool mbs_fc06_hreg_write(struct bcbus_context *ctx)
{
	const uint8_t request_len = 4;
	const uint8_t response_len = 4;
	int err;
 ...
 */
#ifdef CONFIG_BCBUS_FC08_DIAGNOSTIC
static bool mbs_fc08_diagnostics(struct bcbus_context *ctx)
{
	const uint8_t request_len = 4;
	const uint8_t response_len = 4;
	uint16_t sfunc;
 ...
 *  Quantity of Outputs   2 Bytes
 */
static bool mbs_fc15_coils_write(struct bcbus_context *ctx)
{
	const uint16_t coils_limit = 2000;
 ...
 *  Quantity of Registers 2 Bytes
 *
 * If the address of the request exceeds or is equal to BCBUS_FP_EXTENSIONS_ADDR,
 ...
 */
static bool mbs_fc16_hregs_write(struct bcbus_context *ctx)
{
	const uint16_t regs_limit = 125;
	const uint8_t request_len = 6;
	const uint8_t response_len = 4;
